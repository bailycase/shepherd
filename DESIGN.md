...
The canvas's older **Foundations** and **Components** boards (removed from the canvas in v97) and
its `tokens.json` are Option A, superseded by Night Watch. Take no value, name, or component from
them: IBM Plex Sans and
...
Subagents, SubagentsDone, Review); when both exist, the inspector wins. A tray row, a record
line, and the footer's "3 subagents" open the inspector; the tray's Steer opens it with its
Steer field focused. It sits at the
...
- **One run** (MobileSubagent; `SubagentRunScreen`): the name over "Running · 37m", and ••• (Pause
  ...
  - Its transcript: prose at 15/1.5, activity lines 32pt tall at 14; the running call live with its
    ...
    the thread; nothing shows between calls (nothing in a transcript spins). The app shows a live
    call as the run's working row ("Running bash swift build…") without the output tail: the
    child's session holds no streamed output.
...
| SubagentsDone | Subagents; Right pane › Subagent inspector | Partial |
