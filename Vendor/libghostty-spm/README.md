...
> **Vendored copy.** This is a trimmed vendored snapshot of
> [Lakr233/libghostty-spm](https://github.com/Lakr233/libghostty-spm) (MIT, see
> [LICENSE](LICENSE)) carrying only the `GhosttyKit` and `GhosttyTerminal` targets Shepherd
> uses. `Ghostty.version` records the upstream release whose prebuilt
> `GhosttyKit.xcframework` the package downloads; that binary is built from
> [Ghostty](https://ghostty.org), which has its own license terms. Example apps and other
> products mentioned below live in the upstream repository, not in this snapshot.
