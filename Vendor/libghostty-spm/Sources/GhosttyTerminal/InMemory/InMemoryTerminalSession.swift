...
public final class InMemoryTerminalSession: @unchecked Sendable {
    ...
    public func receive(_ data: Data) {
        receive(data, completion: nil)
    }

    /// Enqueue host output and invoke `completion` after Ghostty has processed
    /// it on the same generation-bound serial surface queue.
    public func receive(_ data: Data, completion: (@Sendable () -> Void)?) {
        surfaceAccess.enqueueWrite(data)
        if let completion {
            _ = surfaceAccess.enqueueSurfaceOperation { _ in completion() }
        }
        TerminalDebugLog.log(
            .output,
    ...
    /// Apply a precision scroll directly to the attached local Ghostty mirror.
    /// The operation shares the ordered surface queue with host output, so a
    /// local gesture cannot race a render-frame replay or surface replacement.
    @discardableResult
    public func scroll(x: Double = 0, y: Double) -> Bool {
        surfaceAccess.enqueueSurfaceOperation { surface in
            ghostty_surface_mouse_scroll(
                surface,
                x,
                y,
                TerminalScrollModifiers(precision: true).rawValue
            )
        }
    }

    @discardableResult
    public func scrollToRow(_ row: UInt64) -> Bool {
        surfaceAccess.enqueueSurfaceOperation { surface in
