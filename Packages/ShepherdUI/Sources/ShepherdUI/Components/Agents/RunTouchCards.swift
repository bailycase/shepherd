...
public struct NWRunCard: View, Equatable {
    ...
    private var summary: some View {
        ...
        return VStack(alignment: .leading, spacing: NW.Space.m) {
            HStack(spacing: NW.Space.m) {
                ...
                Text(run.name).font(.nwMono(NWTextStyle.ui.size, .semibold)).foregroundStyle(nw.textPrimary).lineLimit(1)
                    .layoutPriority(2)
                if let tags = run.tags {
                    Text(tags).font(.nw(.caption)).foregroundStyle(nw.textTertiary).lineLimit(1).layoutPriority(0)
                }
                Spacer(minLength: NW.Space.s)
                let pill = pill
                NWRunPill(run.state, label: pill.label, since: pill.since, until: pill.until)
                    .layoutPriority(3)
            }
            switch run.state {
            case .running:
                if run.step != nil || run.progress != nil || run.tokens != nil {
                    HStack(spacing: NW.Space.m) {
    ...
    private var detailLine: some View {
        Text(run.detail).font(.nw(.mono)).foregroundStyle(Color.nw.textSecondary).lineLimit(1).truncationMode(.middle)
    }
...
public struct NWRunGroupCard: View, Equatable {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let footer, !finished {
                HStack(spacing: NW.Space.m) {
                    ...
                    Text(footer).font(.nw(.caption)).foregroundStyle(nw.textTertiary).lineLimit(1)
                }
    ...
    private var header: some View {
        ...
        return HStack(spacing: NW.Space.m) {
            ...
            if let status {
                Text(status).font(.nw(.mono)).foregroundStyle(nw.textTertiary).lineLimit(1)
            }
            if let openAll {
    ...
    private func liveRow(_ row: NWRunGroupRow) -> some View {
        ...
        return HStack(spacing: NW.Space.m) {
            ...
            Text(row.name).font(.nwMono(NWTextStyle.caption.size + 1, .semibold)).foregroundStyle(nw.textPrimary).lineLimit(1)
                .layoutPriority(1)
            Text(row.detail).font(.nw(.caption)).foregroundStyle(row.state == .attention ? nw.lanternText : nw.textSecondary)
                .lineLimit(1).truncationMode(.tail)
                .frame(maxWidth: .infinity, alignment: .leading)
            if let since = row.since {
                NWElapsedText(since: since, until: row.until).font(.nw(.mono)).foregroundStyle(nw.textTertiary)
            }
        }
        .padding(.horizontal, NW.Space.l)
        .frame(minHeight: NW.Height.touch)
