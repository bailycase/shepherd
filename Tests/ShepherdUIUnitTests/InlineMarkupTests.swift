...
struct InlineMarkupTests {
    @Test func codeAndOptionNamesBecomeTheirOwnPieces() {
        #expect(NWInlineMarkupParser.parse("passes no `--model` at all.") == [
            .plain("passes no "), .code("--model"), .plain(" at all."),
        ])
        #expect(NWInlineMarkupParser.parse("**Remote default** starts clean. **Current branch** stacks.") == [
    ...
    func textWithoutMarkupStaysOnePlainPiece(source: String) {
        #expect(NWInlineMarkupParser.parse(source) == [.plain(source)])
    }

    /// Underscores inside a code span are the tool's name, never emphasis.
    @Test func codeKeepsItsCharacters() {
        #expect(NWInlineMarkupParser.parse("with `review_diff`.") == [.plain("with "), .code("review_diff"), .plain(".")])
        #expect(NWInlineMarkupParser.parse("Runs `pi update --extensions` once a day.") == [
