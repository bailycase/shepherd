...
struct ProtocolTests {
    ...
    @Test func childCardFieldsDecodeAndRoundTrip() throws {
        ...
        #expect(rows[2].files == [ChildFileChange(path: "Tests/A.swift", added: 96, removed: 3)] && rows[2].summary == "Added 6 tests.")
        #expect(rows[2].sessionID == "child-3" && rows[2].cwd == "/repo" && rows[0].files == nil && rows[0].sessionID == nil)
