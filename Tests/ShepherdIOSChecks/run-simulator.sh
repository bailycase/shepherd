...
xcrun swiftc -sdk "$(xcrun --sdk iphonesimulator --show-sdk-path)" \
    ...
    App/iOS/HostConnection.swift App/iOS/MobileTokens.swift App/iOS/Thread*.swift \
        ...
        with conn, conn.makefile('r') as stream:
            for line in stream:
                ...
                if kind == 'hello':
                    ...
                    reply = dict(type='state', id=rid, state=dict(spaces=[], tabs=[], agents=[dict(id='fixture-agent', name='verify native thread', spaceID='fixture-space', tabID='fixture-tab', status='blocked')]))
                elif kind == 'nativeThread':
                    action = request['request']
                    assert 'snapshot' in action, 'fixture unexpectedly mutated'
                    snap = dict(piSessionID='fixture-session', generation='fixture-generation', revision=1, running=True,
...
try:
    ...
    env = dict(os.environ, SIMCTL_CHILD_FIXTURE_PORT=str(listener.getsockname()[1]), SIMCTL_CHILD_FIXTURE_SCHEME=os.environ.get('FIXTURE_SCHEME', 'dark'))
