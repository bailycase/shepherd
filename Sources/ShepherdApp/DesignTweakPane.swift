...
/// scope when the element has a name, and Reset and "Ask the agent instead…" pinned under them.
...
private struct DesignTweakRowView: View {
    ...
    var body: some View {
        ...
        case .text(let text):
            NWTweakRow(row.label) {
                DesignTweakTextField(label: row.label, text: text) { typed in
    ...
    /// The last value a drag reached, for its release.
...
private struct DesignTweakTextField: View {
    let label: String
    let text: String
    ...
    var body: some View {
        TextField(label, text: $draft)
            ...
            .focused($focused)
            .frame(width: AppLayout.designTweakFieldWidth)
            .onAppear { draft = text }
