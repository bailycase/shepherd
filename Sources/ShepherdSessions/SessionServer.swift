...
public final class SessionServer: @unchecked Sendable {
    ...
    /// Forgets a design and removes its folder. Its agent goes with it (with its layout and
    /// processes): it lives in the reserved designs space, where nothing else reaches it.
    public func deleteDesign(_ designID: DesignID) async throws {
        try await enqueue {
            ...
            let designSpace = state.designsSpace?.id
            let doomedAgents = state.agents.filter { $0.designID == designID && $0.spaceID == designSpace }
            let doomedIDs = Set(doomedAgents.map(\.id))
            let doomedTabs = state.tabs.filter { tab in
                doomedAgents.contains { $0.tabID == tab.id } || tab.inspectorFor.map(doomedIDs.contains) == true
            }
            let doomedTabIDs = Set(doomedTabs.map(\.id))
            let sessions = Set(doomedTabs.flatMap { $0.layout.leaves.compactMap(\.sessionID) })
            try self.mutateState {
                $0.designs.removeAll { $0.id == designID }
                $0.agents.removeAll { doomedIDs.contains($0.id) }
                $0.tabs.removeAll { doomedTabIDs.contains($0.id) }
                for i in $0.agents.indices where $0.agents[i].designID == designID {
                    $0.agents[i].designID = nil
                }
                for i in $0.automations.indices where $0.automations[i].agentID.map(doomedIDs.contains) == true {
                    $0.automations[i].agentID = nil
                }
            }
            for sessionID in sessions {
